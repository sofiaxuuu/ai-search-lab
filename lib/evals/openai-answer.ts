...
const CODE_PROXY_INSTRUCTIONS = `Answer the repository-level software question using only the supplied source excerpts.
Be concise but include every implementation detail needed to answer the question correctly.
If the excerpts do not support an answer, return exactly: Insufficient evidence`;
...
export async function answerBenchmarkWithOpenAI(
  apiKey: string,
  benchmark: SingleTurnBenchmarkId,
  item: BenchmarkItem,
  excerpts: RetrievalExcerpt[],
): Promise<AnswerTrace> {
  if (benchmark !== "sweqa-code-proxy") return answerWithOpenAI(apiKey, item, excerpts);
  ...
  const response = await fetch(OPENAI_RESPONSES_URL, {
    ...
    body: JSON.stringify({
      model: ANSWER_MODEL,
      instructions: CODE_PROXY_INSTRUCTIONS,
      input,
      temperature: 0,
  ...
  if (!response.ok) {
    const error = payload.error as { message?: unknown } | undefined;
    throw new Error(
      `OpenAI Responses API failed: ${typeof error?.message === "string" ? error.message : `HTTP ${response.status}`}`,
    );
  }
  const text = extractOutputText(payload);
  if (!text) throw new Error("OpenAI response contained no output text");
  ...
  return {
    ...
    model: typeof payload.model === "string" ? payload.model : ANSWER_MODEL,
    responseId: String(payload.id ?? ""),
    promptVersion: "sweqa-grounded-answer-v1",
    text,
    usage: {
      inputTokens: Number(usage.input_tokens ?? 0),
      outputTokens: Number(usage.output_tokens ?? 0),
