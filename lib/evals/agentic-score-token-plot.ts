...
export function renderAgenticScoreTokenPlot(summary: AgenticSummary) {
  ...
  const maxTokens = Math.max(...entries.flatMap(([, value]) => [value.standardObservedTokens.mean, value.dynamicObservedTokens.mean])) * 1.12;
  ...
  const marks = entries.map(([benchmark, value], index) => {
    ...
    const sx = x(value.standardObservedTokens.mean);
    const sy = y(value.standardScore);
    const dx = x(value.dynamicObservedTokens.mean);
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc"><title id="title">Local agentic score versus observed tokens</title><desc id="desc">Diamonds show Standard Highlights and circles show Dynamic Highlights for four agentic benchmark pilots.</desc><rect width="100%" height="100%" fill="#fbfaf7"/><g font-family="ui-sans-serif,system-ui,sans-serif"><text x="${margin.left}" y="38" font-size="24" font-weight="700" fill="#20242c">Local agentic score vs. total observed tokens</text><text x="${margin.left}" y="64" font-size="13" fill="#626a64">Diamond = Standard · Circle = Dynamic · lines pair the same benchmark sample</text>${xTicks}${yTicks}<line x1="${margin.left}" y1="${margin.top + plotHeight}" x2="${margin.left + plotWidth}" y2="${margin.top + plotHeight}" stroke="#20242c"/><line x1="${margin.left}" y1="${margin.top}" x2="${margin.left}" y2="${margin.top + plotHeight}" stroke="#20242c"/>${marks}<text x="${margin.left + plotWidth / 2}" y="${height - 24}" text-anchor="middle" font-size="15" fill="#20242c">Mean total observed tokens per question</text><text transform="translate(24 ${margin.top + plotHeight / 2}) rotate(-90)" text-anchor="middle" font-size="15" fill="#20242c">Benchmark score</text></g></svg>`;
}
